	...
	public static void load() throws Exception {
		...
		for (File f : it) {
			...
			if (f.getName().toLowerCase().endsWith(".jar")) {
				System.out
						.println("add plugin " + f.getAbsolutePath() + " \t " + sdf.format(new Date(f.lastModified())));
				jars.add(f.toURI().toURL());
			}
		}
		System.out.println("added plugin count:" + jars.size());
		cl = new URLClassLoader(jars.toArray(new URL[jars.size()]), Plugin.class.getClassLoader());
