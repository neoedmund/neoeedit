...
/**
 * use lib rhino JS lib seem not stable on packages, not same in oraclejdk,
 * openjdk. If it keeps like this, it will be removed later.
 */
public class JS {
	...
	// }
	//
	// public static List<StringBuffer> run_WithScriptEngineManager(
	// List<StringBuffer> lines, String userScript) throws Exception {
	...
	// }
	//
	// static String funcWrappedRun = "function wrappedRun(a,b,c){var
	// ret=run(a,b,c);if (ret==undefined) {return ret;} if (ret.constructor ==
	// Array){"
	// + " var jArr = java.lang.reflect.Array.newInstance(java.lang.String,
	// ret.length);"
	// + " for (var i = 0; i < ret.length; i++) jArr[i] = ret[i];"
	// + " return jArr;" + "}else{return ret;}}";
	//
	// public static List<StringBuffer> run_WithRhino(List<StringBuffer> lines,
	// String userScript) throws Exception {
	// Class cxCls = null;
	// Class funcCls = null;
	...
	// clsScriptable = Plugin.cl
	// .loadClass("org.mozilla.javascript.Scriptable");
	// } catch (ClassNotFoundException ex) {
	// throw new RuntimeException(
	// "Cannot find Rhino(org.mozilla.javascript.*), maybe you should copy js.jar
	// into dir '<home>/.neoeedit/plugins'.");
	// }
	//
	...
	// try {
	//
	// Object scope = cxCls.getMethod("initStandardObjects",
	// new Class[] {}).invoke(cx, null);
	//
	// Method evaluateString = findMethod1(cxCls, "evaluateString", 5);
	// evaluateString.invoke(cx, new Object[] { scope, funcWrappedRun,
	// "<1>", 1, null });
	// evaluateString.invoke(cx, new Object[] { scope, userScript,
	// "<1>", 1, null });
	//
	// Method funcCall = findMethod1(funcCls, "call", 4);
	// Object funcObj = clsScriptable.getMethod("get",
	// new Class[] { String.class, clsScriptable }).invoke(
	// scope, new Object[] { "wrappedRun", scope });
	//
	// List<StringBuffer> res = new ArrayList<StringBuffer>();
	// int total = lines.size();
	// for (int i = 0; i < total; i++) {
	// Object o = funcCall.invoke(funcObj,
	// new Object[] {
	// cx,
	// scope,
	// scope,
	// new Object[] { lines.get(i).toString(), i,
	// total } });
	// addResult(o, res);
	// }
	// if (res.size() == 0) {
	...
	// { for (Method m : cls.getMethods()) { if (m.getName().equals(name)
	// && (m.getParameterTypes().length == paramCnt)) {
	// return m;
	// }
	// }
	// return null;
